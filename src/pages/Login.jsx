    ...
    <>
      <section className="fixed text-gray-300 flex justify-center items-center flex-col top-0 z-30 bg-[#1d1e20] h-screen w-screen" >
        ...
        <form className="max-w-screen-sm  flex flex-col gap-3 items-center p-4 m-2 w-screen transition-all duration-500 " >
          ...
          <input placeholder="Enter Password" type={`${!showPass ? 'password' : 'text'}`} className={`${inputClass}`} name="" id="" />
          <label className="text-gray-300 w-[310px] text-xs flex justify-start items-center" htmlFor="showpassword">
            <input checked={showPass} onChange={(e)=>{setShowPass(e.target.checked)}} id="showpassword" className="mr-1" type="checkbox" />
            Show Password
          </label>
          ...
          <span className="text-xs text-gray-300" href="#">New user? <a href="/register" className="underline">Create Account</a></span>
        </form>
