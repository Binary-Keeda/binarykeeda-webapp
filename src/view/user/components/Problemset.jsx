...
export default function ProblemSet() {
  ...
  return (
    <div className='flex  w-full gap-8'>
      {
        !showProgress && <div onClick={() => { setShowProgress(true) }} className='fixed p-2 cursor-pointer z-40 right-0 bg-sky-600 text-white rounded-full'>
          <MenuOpen sx={{
            cursor: "pointer",
            '& .MuiPaper-root': {
      ...
      <div className='flex-1 flex flex-col gap-3'>
        {problemset.map((topic, index) => (
          ...
          >
            <h3 className='text-xl px-4 border-slate-100 dark:bg-gray-800  bg-slate-100 dark:text-gray-50 py-7 font-bold leading-none text-gray-800'>
                ...
                >
                  ...
                  </p>
                  {
                    ...
                    <div className='flex px-8 gap-10'>
                      ...
                      <input
                        checked={!!localStorage.getItem(`${task.title}+${topic.topicName}`)}
                        type='checkbox'
                        className='cursor-pointer'
                        onChange={() => selectTask(task?.title, topic.topicName)}
                      />
                    </div>
                  }
                </div>
              ...
              {topic.problems?.map((problem, idx) => (
                ...
                >
                  ...
                  <div className='flex px-8 gap-10'>
                    ...
                    <input
                      checked={!!localStorage.getItem(`${problem.title}+${topic.topicName}`)}
                      type='checkbox'
                      className='cursor-pointer'
                      onChange={() => selectQuestion(problem?.title, topic.topicName)}
                    />
                  </div>
                </div>
              ))}
      ...
      <div className={`transition-all duration-200  ${showProgress ? "w-[350px] " : "w-0 "} relative top-[0px] h-[calc(100vh-70px)]`}>
        <aside className={`transition-all flex flex-col justify-between pb-6 right-0  duration-200 ${showProgress ? "translate-x-0" : "translate-x-full"} fixed top-[70px] h-[calc(100vh-70px)] dark:bg-gray-800 dark:text-gray-50 bg-white w-[350px]`}>
          <section className='flex flex-col items-center' >
            <div className='flex w-full items-center justify-between  my-5 gap-4 mx-4'>
              <div onClick={() => { setShowProgress(false) }} className='flex ml-3 cursor-pointer  items-center' >
                <KeyboardArrowRight />
              </div>
            ...
            <Link to={'/user/practice'} className='w-full justify-items-start mx-5 px-6 cursor-pointer' >
                  ...
                  <div className='mx-5'>
                    <div className='flex flex-wrap gap-3'>
                      {Object.entries(problemByTopic).map(([topic, solved]) => (
                        <li key={topic} className='text-sm bg-slate-200 rounded-[20px] list-none p-2 text-gray-700'>
